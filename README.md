...
Google Gemini API** 기반입니다.

## 처리 파이프라인

1. **검색·수집** — 산업군 키워드로 관련 법인기업을 웹에서 검색 (서버에서 Gemini + Google 검색 그라운딩)
...
- **UI (`src/app/page.tsx`, `src/components`)**: 순수 React(클라이언트). 서버 접근은
  **플랫폼 어댑터** 한 곳(`src/platform/index.ts`)으로만 — 컴포넌트는 `fetch`를 직접
  부르지 않습니다.
- **API 라우트 (`src/app/api`)**: 크롤링·검색·파일 처리·Gemini 호출 등 모든 서버
...
- **로직 계층 (`src/lib`)**: `services/*`(인터페이스로 추상화된 Gemini/Node 구현)와
...
> 발급되며(개발 지식 없이 약 2분), 실제 동작에는 **본인 Google Gemini API 키**(무료)가 필요합니다.
...
1. `cp .env.example .env.local` 후 `GEMINI_API_KEY`에 실제 키 입력
...
- 한국 기업정보 공개 API는 제한적이라, 기업 발굴·정보 수집은 Gemini의 **Google 검색
  그라운딩**을 사용합니다. 유료 DB의 무단 크롤링은 하지 않습니다.
- 무료 티어는 웹 검색 품질·호출 한도가 낮아 결과(특히 사업자등록번호·연락처) 정확도가
  낮을 수 있습니다. **실제로 연락하기 전 반드시 직접 재확인**하고, 한 번에 발굴하는 기업
  수는 적게(5개 내외) 두는 것을 권장합니다.
